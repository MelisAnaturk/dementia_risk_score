...
df_demographic <- summary_table(df, summary_demographic, by = c("dementia_BIN_TOTAL"))
#~ qwraps2::mean_sd(, denote_sd = "paren")
#~ qwraps2::n_perc()
...
df_biomed <- summary_table(df, summary_biomed, by = c("dementia_BIN_TOTAL"))
...
df_lifestyle <- summary_table(df, summary_lifestyle, by = c("dementia_BIN_TOTAL"))
